...
# 总结
> epoll fd只能在多线程共享，socket fd可以在多线程多进程下共享，在地址端口重用的情况下，两者都可以在多线程多进程下不共享。在epoll fd共享的情况下，内核可能总是唤醒第一个调epoll_wait的线程，在socket fd共享的情况下，多进程多线程共享相同的连接队列，内核可能总是唤醒第一个调accept或者epol_wait的线程或进程。
> 多进程模型，每个进程独立监听同一个端口（通过SO_REUSEADDR或SO_REUSEPORT），适合CPU密集型任务。多线程模型，多个线程共享同一个套接字，适合I/O密集型任务。混合模型，结合多进程和多线程，进一步优化性能。
