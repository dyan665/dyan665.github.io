...
//	cout << typeid(T).name()<<endl;
    ...
    if  (sizeof(T) == sizeof(uint16_t)) {
        ...
        buffer.insert(buffer.end(), reinterpret_cast<char*>(&netValue), reinterpret_cast<char*>(&netValue) + sizeof(netValue));
...
public:
    ...
    void registerFunction(const string& name, Func func) {
        ...
        functions[name] = [func,this](const vector<char>& buffer) -> vector<char> {
...
private:
    ...
    // 调用rpc函数并序列化返回值返回
    template<typename Func, typename... Args>
    vector<char> serializeResult(Func&& func, const tuple<Args...>& args) {
        auto result = callFunc(forward<Func>(func), args, make_index_sequence<sizeof...(Args)>{});
        vector<char> buffer;
    ...
    auto callFunc(Func&& func, Tuple&& params, index_sequence<I...>){
